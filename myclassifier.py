...
input_layer = tf.keras.layers.Input([244, 244, 3])

conv1 = tf.keras.layers.Conv2D(filters=32, kernel_size=(5, 5), padding='Same', activation='relu')
pool1 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=None)(conv1)
...
out = tf.keras.layers.Dense(512, activation='relu')(dn1)

model = tf.keras.Model(input_layer, out)

model.compile(optimizer='adam', Loss='sparse_categorical_crossentropu', metrics=['accuracy'])
